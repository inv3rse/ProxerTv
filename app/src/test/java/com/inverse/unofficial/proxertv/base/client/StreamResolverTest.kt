...
class StreamResolverTest {
    ...
    fun testStreamcloudResolver() {
        ...
        resolver.resolveStream("http://streamcloud.eu/rma6ijnb58n0")
            .subscribeAssert {
                ...
                assertValue(
                    Stream(
                        "http://cdn8.streamcloud.eu:8080/zpv75jl4lwoax3ptx32ihr7fuses3udb2dpgvrl344izxpszn4p4ldp3nm/video.mp4",
                        ""
                    )
                )
            }
    ...
    fun testMp4UploadStreamResolver() {
        ...
        resolver.resolveStream("http://www.mp4upload.com/embed-sg71lpccevaa.html").subscribeAssert {
            ...
            assertValue(
                Stream(
                    "https://www2.mp4upload.com:282/d/rgxzcgtmz3b4quuofwvbwksoi244w36p4ot4zdpu3sak6gbf5myfbrny/video.mp4",
                    ""
                )
            )
        }
    ...
    fun testDailyMotionResolver() {
        ...
        resolver.resolveStream("http://www.dailymotion.com/embed/video/x5tqens")
            .subscribeAssert {
                ...
                assertValues(
                    Stream(
                        "http://www.dailymotion.com/cdn/H264-1280x720/video/x5tqens.mp4?auth=1501533278-2688-ysrzfgvg-81963c69aacf70e54cc2fa56f209af40",
                        ""
                    ),
                    Stream(
                        "http://www.dailymotion.com/cdn/H264-848x480/video/x5tqens.mp4?auth=1501533278-2688-r8o5lwqa-ab206b484b0ab0de4c77255027008251",
                        ""
                    )
                )
            }
    ...
    private fun StreamResolver.appliesToUrl(url: String) = HttpUrl.parse(url)?.let { appliesToUrl(it) } ?: false

    private fun StreamResolver.resolveStream(url: String) =
        HttpUrl.parse(url)?.let { resolveStream(it) } ?: Observable.empty()
}